...
fn solve(number:i64, mut output:String, mut output2:String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for phase in 0..(10000010 / segment / dir_len)
    {
        ...
        for state in &states
        {
            ...
            loop 
            {
                ...
                for _ in 0..segment
                {
                    let mut offset = s.current  as usize;
                    offset %= dir_seq_len;
                    offset *= dir_interval;
                    ...
                    s.current = ((state.rand_a * seed.a) * s.current + b) % (state.rand_m);
                    ...
                    for current in &values
                    {
                        for dir in dirs
                        {
                            ...
                            if problem.is_road(&next) && !moved[pos]
                            {
                                ...
                                {
                                    score -= problem.heat_map[pos] / (i + 20) as f64 * 20.0;
                                    break 'out;
                                }
                            }
                        }
    ...
    if let Option::Some(state) = best_state
    {
        ...
        writeln!(file1, "{} {} {} {} {}", state.rand_start, segment, state.rand_a, state.rand_m, b)?;
        let mut current = state.rand_start;
        for seed in &state.history
        {
            writeln!(file1, "{}", seed.a)?;
            for _ in 0..segment
            {
                let mut offset = current  as usize;
                offset %= dir_seq_len;
                offset *= dir_interval;
                for dir in &dir_seq[offset..(offset + dir_len)]
                {
                ...
                current = ((state.rand_a * seed.a) * current + b) % state.rand_m;
            }
        }
        file2.flush()?;
    }
